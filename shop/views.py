...
def home(request):
    products = Product.objects.filter(eco_score=9)
...
def product_details(request, cname, pname):
    if (Catagory.objects.filter(name=cname, status=0)):
        if (Product.objects.filter(name=pname, status=0)):
            ...
            # Add these lines for recommendations
            recommender = ProductRecommender()
            recommendations = recommender.get_recommendations(products.id)
            
            # Update your return statement to include recommendations
            ...
            if product_status:
                if Cart.objects.filter(user=request.user.id, product_id=product_id):
                    ...
                    if product_status.quantity >= product_qty:
                        Cart.objects.create(
                            user=request.user, product_id=product_id, product_quantity=product_qty)
